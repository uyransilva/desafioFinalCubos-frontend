...
function ModalEditClient({ openEdit, setOpenEdit, clientData }) {

    const [openAlert, setOpenAlert] = useState(false)
    const [errorCep, setErrorCep] = useState(false)
    const navigate = useNavigate();
    const token = getLocalItem('token')
    const arrayCep = clientData.cep.split('-')
    const formatCep = arrayCep.join('')
    const headers = {
    ...
    function closeModal() {
        ...
        formik.errors.userAdress = ''
        formik.errors.userComplement = ''
        formik.errors.userCep = ''
    ...
    const formik = useFormik({
        initialValues: {
            ...
            userCpf: clientData.cpf,
            userPhone: clientData.telefone,
            userAdress: clientData.logradouro,
            userComplement: clientData.complemento,
            userCep: formatCep,
            userDistrict: clientData.bairro,
            userCity: clientData.cidade,
            userState: clientData.estado
        },
        validate: function (values) {
            ...
            const validateComplement = !isNaN(parseFloat(values.userComplement)) && isFinite(values.userComplement);
            ...
            if (validateComplement) { errors.userComplement = 'Este campo não pode conter apenas números'; }
    ...
    useEffect(() => {
        formik.values.userName = clientData.nome
        formik.values.userEmail = clientData.email
        formik.values.userCpf = clientData.cpf
        formik.values.userPhone = clientData.telefone
        formik.values.userAdress = clientData.logradouro
        formik.values.userComplement = clientData.complemento
        formik.values.userCep = formatCep
        formik.values.userDistrict = clientData.bairro
        formik.values.userCity = clientData.cidade
        formik.values.userState = clientData.estado
    }, [openEdit])
    ...
    async function handleChangeCep() {
        ...
        try {
            ...
            if (error) {
                toastError('Informe um CEP valido')
                setErrorCep(true)
            }
        }
    }
    ...
    async function handleSubmit(event) {
        ...
        try {
            const { data } = await api.put('/clients', {
                ...
                logradouro: replaceWhiteSpaces(formik.values.userAdress.trim()),
                complemento: replaceWhiteSpaces(formik.values.userComplement.trim()),
                cep: formik.values.userCep ? handleFormatCep(formik.values.userCep) : '',
                bairro: replaceWhiteSpaces(formik.values.userDistrict.trim()),
                cidade: replaceWhiteSpaces(formik.values.userCity.trim()),
                estado: replaceWhiteSpaces(formik.values.userState.trim()),
            ...
            {openEdit &&
                ...
                >
                    ...
                    <div className="modalRegister">
                        <h2><img src={ClientIcon} alt="Ícone de clintes"></img>
                            Editar Cliente</h2>
                        <img
                            src={CloseIcon}
                            alt="Ícone de fechar"
                        ...
                        <form onSubmit={handleSubmit} className="modal-form-register">
                            ...
                            <div className='container-cep-district'>
                                <div className='input-register'>
                                    <span>CEP</span>
                                    <input name="userCep" onBlur={formik.handleBlur} onKeyUp={handleChangeCep} onChange={formik.handleCep} type='number' value={formik.values.userCep}
                                        className='input-mask-cep' placeholder='Digite seu CEP' />
